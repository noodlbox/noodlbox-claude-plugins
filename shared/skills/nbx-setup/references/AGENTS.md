...
- Command output is JSON — pipe to `jq` if a script needs to parse it. Exception: `noodl diff` renders a change tree by default; pass `--json` for the machine-readable report.
